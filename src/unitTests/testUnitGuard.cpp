...
//------------------------------------------------------------------------------
// Example Google Test test fixture or direct test
//------------------------------------------------------------------------------
...
TEST(UnitDimensionTests, DerivedUnits)
{
  ...
  using ExpectedEntropy = Unit< PowUnit< MassTag, 1 >, PowUnit< LengthTag, 2 >, PowUnit< TimeTag, -2 >, PowUnit< TemperatureTag, -1 > >;
...
TEST(MetafunctionTests, MultiplyAndDivide)
{
  ...
  using Mul1 = typename Multiply< LengthDimension, Unit< PowUnit< TimeTag, -1 > > >::type;
  ...
  using Div1 = typename Divide< LengthDimension, Unit< PowUnit< TimeTag, -1 > > >::type;
  using Expected = Unit< PowUnit< LengthTag, 1 >, PowUnit< TimeTag, 1 > >;
  ...
  static_assert( std::is_same< InvTemp, Unit< PowUnit< TemperatureTag, -1 > > >::value, "Invert Temperature => Temperature^-1");
...
TEST(QuantityTests, BasicArithmetic)
{
  ...
  // Check type
  static_assert( std::is_same< decltype( area ), AreaType >::value, "Multiplying Length by Length => Area" );
  // Check value
...
TEST(QuantityTests, CrossDimensionArithmetic)
{
  ...
  auto speed = len / dur;
  static_assert( std::is_same< decltype( speed ), Velocity >::value, "Length / Time => Velocity" );
  ...
  static_assert( std::is_same< decltype( entropy ), Entropy >::value, "Energy / Temperature => Entropy" );
